...
#define ES_CVS_TEAR_DOWN_PRESET		0x1d4c
...
#define ES_FW_RECOVERY_INTR_EVENT	0x0008
...
struct escore_priv {
	...
	int sleep_abort;
	bool kw_detected_before_streaming;
	...
	void (*device_recovery)(struct escore_priv *escore);
...
extern int escore_write_block(struct escore_priv *escore,
		const u32 *cmd_block);
int escore_read_locked(struct escore_priv *escore,
			       unsigned int reg);
extern int escore_read(struct escore_priv *escore,
			       unsigned int reg);
int escore_write_locked(struct escore_priv *escore, unsigned int reg,
		       unsigned int value);
extern int escore_write(struct escore_priv *escore, unsigned int reg,
		       unsigned int value);
