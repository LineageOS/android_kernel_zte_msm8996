...
#ifdef DUMP_WAKELOCK
static struct dentry *wakeup_sources_stats_active_dentry;
/**
 * print_wakeup_source_stats - Print wakeup source statistics information.
 * @m: seq_file to print the statistics into.
 * @ws: Wakeup source object to print the statistics for.
 */
static int print_wakeup_source_stats_active(struct seq_file *m,
				     struct wakeup_source *ws)
{
	unsigned long flags;
	ktime_t total_time;
 ...
 * @m: seq_file to print the statistics into.
 */
static int wakeup_sources_stats_active_show(struct seq_file *m, void *unused)
{
	struct wakeup_source *ws;
	...
	rcu_read_lock();
	list_for_each_entry_rcu(ws, &wakeup_sources, entry)
		print_wakeup_source_stats_active(m, ws);
	rcu_read_unlock();

	return 0;
}

static int wakeup_sources_stats_active_open(struct inode *inode, struct file *file)
{
	return single_open(file, wakeup_sources_stats_active_show, NULL);
}

static const struct file_operations wakeup_sources_stats_active_fops = {
	.owner = THIS_MODULE,
	.open = wakeup_sources_stats_active_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
#ifdef DUMP_WAKELOCK
	wakeup_sources_stats_active_dentry = debugfs_create_file("wakeup_sources_active",
			S_IRUGO, NULL, NULL, &wakeup_sources_stats_active_fops);
#endif
	return 0;
}
