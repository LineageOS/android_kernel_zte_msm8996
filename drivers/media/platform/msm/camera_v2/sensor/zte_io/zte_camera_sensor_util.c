...
static int ois_debugfs_datatype_s(void *data, u64 val)
{
	...
	CDBG("%s:%d: msm_ois_reg_data_type = %d", __func__, __LINE__,
...
static int ois_debugfs_setvalue(void *data, u64 val)
{
	...
	rc = ois_i2c_client->i2c_func_tbl->i2c_write(
			...
			ptr->msm_ois_reg_data_type);
	if (rc < 0) {
		pr_err("%s:%d: i2c write failed", __func__, __LINE__);
		return rc;
	}
...
static int ois_debugfs_getvalue(void *data, u64 *val)
{
	...
	rc = ois_i2c_client->i2c_func_tbl->i2c_read(
			...
			ptr->msm_ois_reg_data_type);
	if (rc < 0) {
		pr_err("%s:%d: i2c read failed", __func__, __LINE__);
		return rc;
	}
...
int msm_ois_enable_debugfs(struct msm_ois_ctrl_t *s_ctrl)
{
	...
	ois_dir = debugfs_create_dir(s_ctrl->msm_sd.sd.name, ois_debugfs_base);
	if (!ois_dir) {
		pr_err("%s:%d: exit", __func__, __LINE__);
		goto debug_ptr_free;
	}
...
static ssize_t show_msm_sensor_name(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (msm_sensor_sysdev_info[index].name != NULL)
		len += snprintf(buf+len,  strlen(msm_sensor_sysdev_info[index].name) + 3,
