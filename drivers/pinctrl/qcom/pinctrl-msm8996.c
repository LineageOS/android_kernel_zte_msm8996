...
extern unsigned gpio_level_store_pm(int *id,struct gpio_chip *chip,unsigned values);
...
int  gpio_status_debug_init  (struct gpio_chip *chip)
{
	...
	if (!debugfs_create_file("dump_gpios", S_IRUGO, debugfs_base,NULL, &list_gpios_fops))
		return -ENOMEM;

	if (!debugfs_create_file("dump_sleep_gpios", S_IRUGO, debugfs_base,NULL, &list_sleep_gpios_fops))
		return -ENOMEM;
