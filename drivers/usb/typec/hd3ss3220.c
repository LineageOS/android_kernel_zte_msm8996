                    ...
                    struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    //info->type_c_param.cc_mode = value;
...
static int  hd3ss3220_suspend(struct device *dev)
{
    ...
    // disable id irq in qpnp charger
    //interfere_id_irq_from_usb(0);

    // set UFP [0x01] mode, REG_SET[bit 5:4]
    //ret = hd3ss3220_update_reg(info->i2c, REG_SET,
        //(SET_MODE_SELECT_SNK << SET_MODE_SELECT_SHIFT), SET_MODE_SELECT);
    //if (ret < 0){
        //pr_err("%s: update reg fail!\n", __func__);
    //}
...
static int  hd3ss3220_resume(struct device *dev)
{
    ...
    // restore cc mode, REG_SET[bit 5:4]
    //ret = hd3ss3220_update_reg(info->i2c, REG_SET,
        //(info->type_c_param.cc_mode << SET_MODE_SELECT_SHIFT), SET_MODE_SELECT);
    //if (ret < 0){
        //pr_err("%s: update reg fail!\n", __func__);
    //}

    // enable id irq in qpnp charger
    //interfere_id_irq_from_usb(1);
...
static int hd3ss3220_initialization(struct hd3ss3220_info *info)
{
     ...
     */
    //info->type_c_param.cc_mode = SET_MODE_SELECT_DEFAULT;
    reg_val = 0x02;
