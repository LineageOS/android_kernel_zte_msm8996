...
static s32 gt1x_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			error=GTP_GPIO_OUTPUT(GTP_INT_PORT, gt1x_wakeup_level);
			printk("pingzhenghai:GTP_GPIO_OUTPUT---GTP_INT_PORT=%d   error=%d\n",GTP_INT_PORT,error);
			msleep(5);

    		/* Synchronize int IO */
    		error=GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
		printk("pingzhenghai:GTP_GPIO_OUTPUT---GTP_INT_PORT=%d   error=%d\n",GTP_INT_PORT,error);
    		msleep(50);
    		GTP_GPIO_AS_INT(GTP_INT_PORT);
            flag = 1;
...
/*ergate*/
#ifdef CONFIG_FORCE_TOUCH_NDT
				/*1.skip switching from non-touching to touching*/
				/*2.handle only first finger*/
				if(CHK_BIT(pre_event, BIT_TOUCH) && report_num == 0)
				{//first point
					ret = ndt_stress_set_vlaue(input_x, input_y, 1);
					if(ret == 0)
					{
						ndt_stress_get_vlaue(&ft_value);
						input_w = ft_value;
						printk("ft_value:%d\n",ft_value);
					}
				}
...
void gt1x_resume(struct work_struct *work)
{
	s32 ret = -1;
