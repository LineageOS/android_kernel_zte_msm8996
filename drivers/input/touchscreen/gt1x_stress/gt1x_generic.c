...
static ssize_t gt1x_debug_write_proc(struct file *, const char *, size_t, loff_t *);
...
s32 gt1x_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
	...
	if (len > 16) {
		//GTP_ERROR("i2c_read_dbl_check length %ld is too long, do not beyond %d", len, sizeof(buf));
		return ERROR;
	}
...
s32 gt1x_init_panel(void)
{
	...
	const u8 cfg_grp0_gloved[] = GTP_CFG_GROUP0_GLOVED;
	const u8 cfg_grp1_gloved[] = GTP_CFG_GROUP1_GLOVED;
	const u8 cfg_grp2_gloved[] = GTP_CFG_GROUP2_GLOVED;
	const u8 cfg_grp3_gloved[] = GTP_CFG_GROUP3_GLOVED;
	const u8 cfg_grp4_gloved[] = GTP_CFG_GROUP4_GLOVED;
	const u8 cfg_grp5_gloved[] = GTP_CFG_GROUP5_GLOVED;
	const u8 *cfgs_gloved[] = {
		cfg_grp0_gloved, cfg_grp1_gloved, cfg_grp2_gloved,
		cfg_grp3_gloved, cfg_grp4_gloved, cfg_grp5_gloved
	};
	u8 cfg_lens_gloved[] = {
		CFG_GROUP_LEN(cfg_grp0_gloved),
		CFG_GROUP_LEN(cfg_grp1_gloved),
		CFG_GROUP_LEN(cfg_grp2_gloved),
		CFG_GROUP_LEN(cfg_grp3_gloved),
		CFG_GROUP_LEN(cfg_grp4_gloved),
		CFG_GROUP_LEN(cfg_grp5_gloved)
	};
#endif /* end  GTP_CHARGER_SWITCH */
...
#ifdef CONFIG_CREATE_GLOVED_INTERFACE
	GTP_DEBUG("gloved Config Groups Length: %d, %d, %d, %d, %d, %d", cfg_lens_gloved[0],
		   cfg_lens_gloved[1], cfg_lens_gloved[2], cfg_lens_gloved[3], cfg_lens_gloved[4], cfg_lens_gloved[5]);

	memset(gt1x_config_gloved, 0, sizeof(gt1x_config_gloved));
	if (cfg_lens_gloved[sensor_id] == cfg_len) {
		memcpy(gt1x_config_gloved, cfgs_gloved[sensor_id], cfg_len);
	}

	/* clear the flag, avoid failure when send the config of driver. */
	gt1x_config_gloved[0] &= 0x7F;

#if GTP_CUSTOM_CFG
	gt1x_config_gloved[RESOLUTION_LOC] = (u8) GTP_MAX_WIDTH;
	gt1x_config_gloved[RESOLUTION_LOC + 1] = (u8) (GTP_MAX_WIDTH >> 8);
	gt1x_config_gloved[RESOLUTION_LOC + 2] = (u8) GTP_MAX_HEIGHT;
	gt1x_config_gloved[RESOLUTION_LOC + 3] = (u8) (GTP_MAX_HEIGHT >> 8);

	if (GTP_INT_TRIGGER == 0) {	/* RISING  */
		gt1x_config_gloved[TRIGGER_LOC] &= 0xfe;
	} else if (GTP_INT_TRIGGER == 1) {	/* FALLING */
		gt1x_config_gloved[TRIGGER_LOC] |= 0x01;
	}
#endif /* END GTP_CUSTOM_CFG */
	if (cfg_lens_gloved[sensor_id] != cfg_len) {
		memset(gt1x_config_gloved, 0, sizeof(gt1x_config_gloved));
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	memset(&tsc_id_info, 0, sizeof(struct tsc_id_t));
	memcpy(tsc_id_info.product_id, product_id, strlen(product_id) > sizeof(product_id) ? sizeof(product_id) : strlen(product_id) );
	tsc_id_info.patch_id = patch_id;
	tsc_id_info.sensor_id = sensor_id;
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				if(report_num == 0){//first point
					if(!ndt_stress_set_vlaue(input_x, input_y, 1))
						{
						ndt_stress_get_vlaue(&vlaue1);
						input_z = vlaue1;
						}
					else
						input_z = input_w;
				...
				gt1x_touch_down(input_x, input_y, input_z, input_w, i);
...
#ifdef CONFIG_CREATE_GLOVED_INTERFACE
u8 gt1x_config_gloved[GTP_CONFIG_MAX_LENGTH] = { 0 };
int current_gloved_finger_state = 0;
int g_gloved_flag = 0;

void write_gloved_value(u8 value)
{
	//unsigned char gloved_mode = 0;
	
	//current_gloved_finger_state = value;

	if (value == 0){
		GTP_INFO("non-gloved config sending.....");
		if (!gt1x_send_cfg(gt1x_config, gt1x_cfg_length)) {
			g_gloved_flag = value;
		}
		else{
		...
		GTP_INFO("gloved config sending.....");
		if (!gt1x_send_cfg(gt1x_config_gloved, gt1x_cfg_length)) {
			g_gloved_flag = value;
		}
		else{
			GTP_ERROR("Send config for Charger Plugin failed!");
		...
		printk("XXXXX set gloved mode value error!!!! XXXXX");
	
}

static int gloved_finger_proc_show(struct seq_file *m, void *v)
{
	//unsigned char gloved_mode = 0;
	//struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data; 

	seq_printf(m, "In suspend: current_gloved_finger_state:%d, g_gloved_flag %d.\n", current_gloved_finger_state, g_gloved_flag);


	return 0;
}

static int gloved_finger_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, gloved_finger_proc_show, NULL);
}

static ssize_t gloved_finger_proc_write(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	...
	if (input == current_gloved_finger_state)
		return 1;

	current_gloved_finger_state = input;

	printk("********************[%s] [%d] current_gloved_finger_state %u\n", __func__, __LINE__, current_gloved_finger_state);
...
static const struct file_operations gloved_finger_proc_fops = {
	.open		= gloved_finger_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release		= single_release,
	.write		= gloved_finger_proc_write,
};

int gloved_finger_proc_init(void)
{
       struct proc_dir_entry *res;
	printk("%s %d \n", __func__, __LINE__);
	res = proc_create("gloved_finger_switch", S_IWUGO | S_IRUGO, NULL,
			  &gloved_finger_proc_fops);
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/gloved_finger_switch\n");
		return -ENOMEM;
	}

	printk(KERN_INFO "created /proc/gloved_finger_switch\n");
	return 0;
}
#endif
