...
#define NDT_INFO(fmt, arg...)	pr_notice("<<NDT-INF>>[%s: %d] "fmt"\n",\
									__func__, __LINE__, ##arg)
#define NDT_ERROR(fmt, arg...)	pr_notice("<<NDT-ERR>>[%s: %d] "fmt"\n", __func__, __LINE__, ##arg)
#define NDT_DEBUG(fmt, arg...)	do {\
									if (NDT_DEBUG_ON)\
										pr_notice("<<NDT-DBG>>[%s: %d]"fmt"\n",\
										__func__, __LINE__, ##arg);\
									} while (0)

#define NDT_DEBUG_FUNC()		do {\
									if (NDT_DEBUG_FUNC_ON)\
									pr_notice("<<NDT-FUNC>> Func:%s@Line:%d\n",\
										__func__, __LINE__);\
								} while (0)
...
static int ndt_stress_get_base_info(unsigned short x1,  unsigned short y1, unsigned char status)
{
	...
	if (data_no == 8) {
		...
		if (gNdtData->dmode == DEBUG_MODE_LOG)
			NDT_INFO("touch down: %d %d %d %d %d %d %d %d %d %d %d %d %d",
				frame.press, frame.signal_sum, frame.base1,
				frame.base2, frame.base3, frame.base4,
				frame.param, frame.average, frame.x,
				frame.y, frame.is_mid_edge,
				frame.unedge_percent, frame.percent);
	} else
...
static int ndt_fwu_burn_fw(unsigned char *buf, unsigned int len)
{
	...
	if ((len % 128) != 0) {
		...
		return -EINVAL;/* no need remove device, can not return -EINVAL */
	}
...
static int ndt_fwu_start_reflash(void)
{
	...
	NDT_INFO("read fw_data_len: 0x%02x%02x%02x%02x",
		fw_data_len[0], fw_data_len[1], fw_data_len[2], fw_data_len[3]);
	len = (int)((unsigned int)fw_data_len[3] << 0)|((unsigned int)fw_data_len[2] << 8) |
	...
	if ((gNdtData->force_update == false) && (gNdtData->FirmwareVer < 0x100) &&
		(ntohs(file_fw_ver) <= gNdtData->FirmwareVer)) {
		ret = 1;
	...
	fw_data = kmalloc(len, GFP_KERNEL);
	if (fw_data == NULL) {
		ret = -1;
...
static void ndt_force_firmware_update(void)
{
	...
	strlcpy(gNdtData->update_file, NDT_FW_PATH, sizeof(gNdtData->update_file));
...
static void ndt_debug_handle_work(struct work_struct *work)
{
	...
	if ((gNdtData != NULL) && (gNdtData->dmode) && (gNdtData->appear == true)
		...
		if (gNdtData->dmode == DEBUG_MODE_DDATA) {
			ret = ndt_stress_i2c_read(client, 0x60, 1, &temp);
			if (ret < 0)
				goto retry_loop;

			ReadFlag = 16;
			ndt_stress_i2c_write(gNdtData->client, 0x60, 1, &ReadFlag);
			...
			if (ReadFlag == 1) {
				NDT_INFO("***************** Debug value: (%05d) - (%05d) = %d!!!",
					buffer[0], gNdtData->dvalue1, buffer[0] - gNdtData->dvalue1);
				mutex_lock(&(gNdtData->stress_value_mutex));
				...
				for (i = 0; i < CHANNEL_NUM; i++)
					pr_notice("%d\t", buffer[i]);
				pr_notice("\n");
			}

			ndt_stress_i2c_write(gNdtData->client, 0x60, 1, &temp);
...
static int test_proc_show(struct seq_file *m, void *v)
{
	...
	count = ARRAY_SIZE(gNdtData->sensor);
					...
					size_t count, loff_t *pos)
{
	...
	if ((count > sizeof(gNdtData->update_file)) || (count == 0)) {
		NDT_ERROR("write count %u, update file name len %u",
			(unsigned int)count,
			(unsigned int)sizeof(gNdtData->update_file));
		return count;
	}
...
static long dev_i2c_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case NDT_IOC_READ_STATUS:
		...
		if (gNdtData->dmode == DEBUG_MODE_LOG)
			NDT_INFO("touch up:%d %d %d %d %d %d %d %d %d %d %d %d %d",
				gpoint_info.press, gpoint_info.signal_sum, gpoint_info.base1,
				gpoint_info.base2, gpoint_info.base3, gpoint_info.base4,
				gpoint_info.param, gpoint_info.average, gpoint_info.x,
				gpoint_info.y, gpoint_info.is_mid_edge,
				gpoint_info.unedge_percent,
				gpoint_info.percent);
...
static int ndt_i2c_test(int delay_time)
{
	...
	do {
		...
		if (ndt_stress_i2c_read(gNdtData->client, 0x05,
			sizeof(FirmwareVer), (unsigned char *)&FirmwareVer) != -1) {
			if (FirmwareVer != 0) {
...
static int ndt_stress_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ndt_i2c_test(0)) {
		...
		ret = false;
		/* tp can't init when stress sensor firmware is bad and i2c is noisy. we must pown down */
		ndt_stress_power_switch(false, false);
#ifdef CONFIG_NDT_USE_UPDATE
		/*delay 10s force update firmware when filesystem have mounted*/
		ndt_force_firmware_update();
#endif
	} else
		gNdtData->update_finished = true;
#else
