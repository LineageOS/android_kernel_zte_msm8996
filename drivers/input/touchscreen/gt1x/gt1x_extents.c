...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_doze_status) {
		...
		GTP_INFO("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE, doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3]);
		if (ret == 0 && doze_buf[0] != 0) {
			...
			if (len > 0) {
				...
				if (ret < 0) {
					GTP_INFO("Read gesture data failed.");
			...
			if (extra_len > 0) {
				...
				if (ret < 0) {
					GTP_INFO("Read extra gesture data failed.");
					mutex_unlock(&gesture_data_mutex);
					return 0;
				}
			}
			...
			switch(doze_buf[0]){
				case 0xCC:
					GTP_INFO("gesture: double click");
                    code = 0x260;
					break;
				case 0x63:
					GTP_INFO("gesture: letter C");
                    code = 0x261;
					break;
				case 0x77:
					GTP_INFO("gesture: letter W");
                    code = 0x262;
					break;
				case 0x6D:
					GTP_INFO("gesture: letter M");
                    code = 0x263;
					break;
				case 0x6F:
					GTP_INFO("gesture: letter O");
                    code = 0x264;
					break;
				default:
