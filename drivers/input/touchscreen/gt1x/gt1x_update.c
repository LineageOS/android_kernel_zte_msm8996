...
int gt1x_update_prepare(char *filename)
{
	...
	while (retry > 0) {
		...
		if (update_info.firmware == NULL) {
			GTP_INFO("Alloc %ld bytes memory fail.", sizeof(struct fw_info));
			continue;
		} else {
			GTP_INFO("Alloc %ld bytes memory success.", sizeof(struct fw_info));
			break;
		}
	}
	if (retry <= 0) {
...
int gt1x_check_firmware(void)
{
	...
	for (i = 0; i < update_info.firmware->subsystem_count; i++) {
		...
		GTP_INFO("Type: %d", update_info.firmware->subsystem[i].type);
		GTP_INFO("Length: %d", update_info.firmware->subsystem[i].length);
...
int gt1x_update_judge(void)
{
	...
	if ((fw_ver_info.patch_id & 0xFF000) != (ver_info.patch_id & 0xFF000)) {
		/*if(g_boot_ffbm)
		{
			GTP_ERROR("CID is not equal,no need to update in PV-system");
			return ERROR_CHECK;
		}*/
		GTP_ERROR("CID is not equal, need update!");
		return 0;
	}
	...
	/*if(g_boot_ffbm)
	{
		GTP_INFO("No need to update FW in PV-system");
		return ERROR_CHECK;
	}*/
	return 0;
}
...
int __gt1x_hold_ss51_dsp_20(void)
{
	...
	while (retry++ < 2000) {
		...
		GTP_DEBUG("Hold ss51 & dsp confirm 0x4180 failed,value:%d", buf[0]);
	}
	if (retry >= 2000) {
