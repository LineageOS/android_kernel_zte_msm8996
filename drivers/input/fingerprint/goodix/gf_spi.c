  ...
  */
#define pr_fmt(fmt) "[GoodixFp]%s: " fmt, __func__
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_RESET:
		...
		if (retval == 0) {
			gf_hw_reset(gf_dev, delay_ms);
		} else {
			pr_warn("Failed to set reset delay from user. retval = %d\n", retval);
		}
		break;
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	pr_info("irq triggered\n");
		wake_lock_timeout(&gf_wake_lock, 2 * HZ);
#ifdef GF_FASYNC
	if (gf_dev->async)
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
	...
	if (gf_dev->input == NULL) {
		pr_warn("Input device subsytem is null");
		return;
	}
...
#endif
{
	...
	*/
	...
	if (minor < N_SPI_MINORS) {
		...
		pr_info("major: %d minor: %lu\n", SPIDEV_MAJOR, minor);
		gf_dev->devt = MKDEV(SPIDEV_MAJOR, minor);
		dev = device_create(gf_class, &gf_dev->spi->dev, gf_dev->devt,
					gf_dev, GF_DEV_NAME);
		status = IS_ERR(dev) ? PTR_ERR(dev) : 0;
	} else {
		pr_info("no minor number available!\n");
		status = -ENODEV;
	}
	...
	if (status == 0) {
		...
		if (gf_dev->input == NULL) {
			pr_info("Faile to allocate input device.\n");
			status = -ENOMEM;
		}
#ifdef AP_CONTROL_CLK
		...
		wake_lock_init(&gf_wake_lock, WAKE_LOCK_SUSPEND, "goodix_lock");
	...
	pr_info("probe end successful\n");
	return status;

error:
	pr_info("error in probe\n");
	gf_cleanup(gf_dev);
	gf_dev->device_available = 0;
	if (gf_dev->devt != 0) {
